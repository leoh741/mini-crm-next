...
export async function generarResumenPagoPDF(cliente, estadoPagoMes = null) {
  ...
  try {
    // Intentar primero con el logo PNG local
    const logoUrlLocal = '/logo.png';
    console.log('[PDF] Cargando logo PNG local para footer (resumen de pago)...');
    logoBase64Footer = await cargarLogoBase64(logoUrlLocal);
    if (logoBase64Footer) {
      console.log('[PDF] Logo PNG local cargado para footer exitosamente (resumen de pago)');
    } else {
      console.warn('[PDF] Logo PNG local no encontrado, intentando con URL remota...');
      // Fallback a URL remota si el logo local no existe
      const logoUrl = 'https://digitalspace.com.ar/wp-content/uploads/2025/01/Recurso-1.webp';
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    if (logoBase64Footer) {
      ...
      try {
        const logoUrlLocal = '/logo.png';
        const logoBase64Retry = await cargarLogoBase64(logoUrlLocal);
        if (logoBase64Retry) {
          // Obtener dimensiones manteniendo proporciones originales
...
export async function generarPresupuestoPDF(presupuesto) {
  ...
  try {
    ...
    logoBase64Footer = await cargarLogoBase64(logoUrlLocal);
    if (logoBase64Footer) {
      console.log('[PDF] Logo PNG local cargado para footer exitosamente (presupuesto)');
    } else {
      console.warn('[PDF] Logo PNG local no encontrado, intentando con URL remota...');
      // Fallback a URL remota si el logo local no existe
      const logoUrl = 'https://digitalspace.com.ar/wp-content/uploads/2025/01/Recurso-1.webp';
