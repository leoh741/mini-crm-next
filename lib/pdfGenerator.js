...
export async function generarInformePDF(informe, totals) {
  ...
  if (totals && totals.totalsGlobal) {
    ...
    doc.setFont(undefined, 'normal');
    const spendMostrar = tg.spendConImpuestos || tg.spend || 0;
    doc.text(formatearMoneda(spendMostrar), kpiX + 35, kpiY);
    if (informe.porcentajeImpuestos > 0 && tg.spendConImpuestos) {
      yPos = kpiY + 6;
      doc.setFontSize(8);
      doc.setTextColor(100, 100, 100);
      doc.text(`(Base: ${formatearMoneda(tg.spend || 0)} + ${informe.porcentajeImpuestos}% impuestos)`, 20, yPos);
      doc.setTextColor(0, 0, 0);
      doc.setFontSize(9);
    }
  ...
  const nombreLimpio = (informe.titulo || 'Informe').replace(/[^a-z0-9\s]/gi, '').trim().replace(/\s+/g, ' ') || 'Informe';
  const nombreArchivo = `${nombreLimpio}.pdf`;

  // En el navegador, descargar igual que resúmenes de pago / presupuestos
