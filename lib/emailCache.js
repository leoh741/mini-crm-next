...
export async function guardarCorreoEnCache(uid, carpeta, mensaje, incluirContenido = false, bodyStatus = null, lastBodyError = null) {
  try {
    ...
    // Actualizar lastBodyError y lastBodyAttemptAt
    // Si se proporciona un error, actualizar ambos
    ...
    // Si bodyStatus="loading", actualizar lastBodyAttemptAt (pero no lastBodyError)
    else if (finalBodyStatus === 'loading') {
      updateFields.mensaje.lastBodyAttemptAt = new Date();
      updateFields.lastBodyAttemptAt = new Date();
      // Preservar lastBodyError existente si existe
      if (mensaje.lastBodyError !== undefined) {
        updateFields.mensaje.lastBodyError = mensaje.lastBodyError;
        updateFields.lastBodyError = mensaje.lastBodyError;
      }
    } 
    // Preservar valores existentes si no se proporcionan nuevos
    else if (mensaje.lastBodyError !== undefined || mensaje.lastBodyAttemptAt !== undefined) {
      updateFields.mensaje.lastBodyError = mensaje.lastBodyError;
      updateFields.mensaje.lastBodyAttemptAt = mensaje.lastBodyAttemptAt;
      if (mensaje.lastBodyError !== undefined) {
        updateFields.lastBodyError = mensaje.lastBodyError;
      }
      if (mensaje.lastBodyAttemptAt !== undefined) {
        updateFields.lastBodyAttemptAt = mensaje.lastBodyAttemptAt;
      }
    }
