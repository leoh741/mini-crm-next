...
export async function obtenerCorreoDelCache(uid, carpeta, incluirContenido = false) {
  try {
    ...
    // Usar findOne con índice compuesto para búsqueda ultra-rápida
    const cached = await EmailCache.findOne({ 
      uid: Number(uid),
      carpeta: carpeta 
    }).lean();
    ...
    if (cached) {
      ...
      if (incluirContenido && cached.incluyeContenido) {
        console.log(`✅ Correo con contenido encontrado en cache persistente! UID: ${uid}`);
        return cached.mensaje;
      }
    }
