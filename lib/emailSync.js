...
import { obtenerCorreoDelCache, guardarCorreoEnCache, normalizarFecha } from "./emailCache.js";
 ...
 * Maneja arrays, Sets y null/undefined de forma segura
 */
function ensureFlagsWithSeen(flags = []) {
  // Normalizar a array: manejar Set, array, null, undefined
  let flagsArray = [];
  if (flags instanceof Set) {
    flagsArray = Array.from(flags);
  } else if (Array.isArray(flags)) {
    flagsArray = [...flags]; // Crear copia para no mutar original
  } else if (flags) {
    // Si es otro tipo, intentar convertir
    flagsArray = [flags];
  }
  
  // Si ya tiene \Seen, retornar sin modificar
  if (flagsArray.includes('\\Seen')) {
    return flagsArray;
  }
  
  // Agregar \Seen al final
  return [...flagsArray, '\\Seen'];
}
    ...
    return await imapManager.withImapClient(async (client) => {
      ...
      try {
        ...
        if (maxUid === 0) {
          ...
          // 🔄 Descargar contenido completo en segundo plano para los primeros 5 correos
          // Esto mejora la experiencia cuando el usuario abre correos recién sincronizados
          if (mensajesBootstrap.length > 0) {
            const { descargarContenidoCompletoEnSegundoPlano } = await import('./emailRead.js');
            const correosParaPrecargar = mensajesBootstrap.slice(0, 5); // Solo los primeros 5
            descargarContenidoCompletoEnSegundoPlano(correosParaPrecargar, carpeta)
              .catch(err => {
                console.warn(`⚠️ Error descargando contenido en segundo plano: ${err.message}`);
              });
          }
        ...
        console.log(`📥 SYNC incremental: maxUid=${maxUid}, carpeta=${carpeta}`);
        ...
        if (mensajesNuevos.length === 0) {
          console.log(`✅ SYNC incremental: nuevosEncontrados=0, maxUid=${maxUid}`);
          return { nuevos: 0, mensajes: [] };
        }
        
        console.log(`📬 SYNC incremental: maxUid=${maxUid}, nuevosEncontrados=${mensajesNuevos.length}`);
        
        // 🔴 FIX CRÍTICO: Invalidar cache de lista ANTES de leer listaExistente
        // Esto previene merge con lista vieja/corrupta
        console.log(`🧹 CACHE: Invalidando cache de lista para carpeta ${carpeta}`);
        await limpiarCacheListaCarpeta(carpeta);
        if (nombreCarpetaReal !== carpeta) {
          console.log(`🧹 CACHE: Invalidando cache de lista para carpeta real ${nombreCarpetaReal}`);
          await limpiarCacheListaCarpeta(nombreCarpetaReal);
        }
        console.log(`✅ CACHE: Cache invalidado para carpeta ${carpeta}${nombreCarpetaReal !== carpeta ? ` y ${nombreCarpetaReal}` : ''}`);
        ...
        // Ahora listaExistente será vacía o reconstruida desde cache individual (más confiable)
        const listaExistente = await obtenerListaDelCache(carpeta, limit) || [];
        console.log(`📊 CACHE: listaExistenteLen=${listaExistente.length}`);
        
        const listaActualizada = [...mensajesNuevos, ...listaExistente.filter(m => !mensajesNuevos.some(n => n.uid === m.uid))].slice(0, limit);
        console.log(`📊 CACHE: listaActualizadaLen=${listaActualizada.length}`);
        ...
        const verificado = await obtenerListaDelCache(carpeta, limit);
        if (verificado && verificado.length > 0) {
          console.log(`✅ POST: verificadoLen=${verificado.length}, cache actualizado correctamente`);
        } else {
          console.warn(`⚠️ POST: verificadoLen=0, cache no disponible después de actualizar lista`);
        }
        ...
        // 🔄 Descargar contenido completo en segundo plano para los primeros 5 correos nuevos
        // Esto mejora la experiencia cuando el usuario abre correos recién sincronizados
        if (mensajesNuevos.length > 0) {
          const { descargarContenidoCompletoEnSegundoPlano } = await import('./emailRead.js');
          const correosParaPrecargar = mensajesNuevos.slice(0, 5); // Solo los primeros 5
          descargarContenidoCompletoEnSegundoPlano(correosParaPrecargar, carpeta)
            .catch(err => {
              console.warn(`⚠️ Error descargando contenido en segundo plano: ${err.message}`);
            });
