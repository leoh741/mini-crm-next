  ...
  coloresUsados.forEach((etiquetasConMismoColor, color) => {
    if (etiquetasConMismoColor.length > 1) {
      ...
      for (let i = 1; i < etiquetasConMismoColor.length; i++) {
        ...
        // Buscar un color disponible que no esté siendo usado
        ...
        if (colorDisponible) {
          ...
          // pero asegurándonos de que no se repita
          let intentos = 0;
          while (intentos < COLORES_DISPONIBLES.length) {
            const colorIndex = COLORES_DISPONIBLES.indexOf(color);
            const nuevoIndex = (colorIndex + i + intentos) % COLORES_DISPONIBLES.length;
            const nuevoColor = COLORES_DISPONIBLES[nuevoIndex];
            if (!coloresAsignados.has(nuevoColor)) {
          ...
          // Si después de todos los intentos no encontramos un color único, usar el primero disponible
          if (intentos >= COLORES_DISPONIBLES.length) {
            const primerColorDisponible = COLORES_DISPONIBLES.find(c => !coloresAsignados.has(c));
            if (primerColorDisponible) {
              etiquetaColorMap.set(etiqueta, primerColorDisponible);
              coloresAsignados.add(primerColorDisponible);
            }
          }
        }
      }
