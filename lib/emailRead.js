...
async function obtenerUltimosCorreos(carpeta = "INBOX", limit = 10) {
  ...
  try {
    ...
    // OPTIMIZACIÓN: Guardar en cache persistente (en segundo plano, no bloquea)
    guardarListaEnCache(carpeta, mensajesOrdenados, limit)
      .catch(err => {
        console.warn(`⚠️ Error al guardar lista en cache persistente (no crítico): ${err.message}`);
      });
...
async function obtenerCorreoPorUID(uid, carpeta = "INBOX", incluirContenido = false) {
  ...
  // OPTIMIZACIÓN: Verificar cache en memoria primero (muy rápido)
  if (!incluirContenido) {
  ...
  // OPTIMIZACIÓN: Verificar cache persistente en MongoDB (ultra-rápido)
  const cachedPersistente = await obtenerCorreoDelCache(uid, carpeta, incluirContenido);
  if (cachedPersistente) {
    // También guardar en cache en memoria para acceso aún más rápido
    if (!incluirContenido) {
      const cacheKey = `${uid}-${carpeta}`;
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        try {
          // OPTIMIZACIÓN: Timeout más largo cuando se incluye contenido (attachments pueden ser grandes)
          const searchTimeout = incluirContenido ? 2000 : 500; // 2s si hay contenido, 500ms si no
          ...
          if (sequenceNumbers && sequenceNumbers.length > 0) {
            ...
            // Timeout más largo cuando se incluye contenido (attachments pueden ser grandes)
            const fetchTimeout = incluirContenido ? 30000 : 300; // 30s si hay contenido, 300ms si no
            let msg = null;
            try {
      ...
      if (!incluirContenido && correoEncontrado) {
        ...
        console.log(`💾 Correo guardado en cache en memoria (${emailCache.size}/${CACHE_SIZE})`);
      }
      
      // OPTIMIZACIÓN: Guardar en cache persistente MongoDB (en segundo plano, no bloquea)
      if (correoEncontrado) {
        // Guardar en segundo plano para no bloquear la respuesta
