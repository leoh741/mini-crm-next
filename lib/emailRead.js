 ...
 * @param {number} limit - Número máximo de correos a obtener (por defecto 15)
 * @returns {Promise<Array>} Array de correos ordenados del más nuevo al más viejo
 */
async function obtenerUltimosCorreos(carpeta = "INBOX", limit = 15) {
  if (!emailConfig.user || !emailConfig.pass || !emailConfig.host) {
    throw new Error("Configuración de correo incompleta. Verifica las variables de entorno.");
  }
  ...
  try {
    ...
    try {
      // OPTIMIZACIÓN: Usar búsqueda más eficiente
      // Si hay muchos correos, buscar solo los más recientes directamente
      // Usar una búsqueda limitada para mejorar el rendimiento
      ...
      // Obtener solo los últimos 'limit' mensajes (los más recientes)
      // Esto evita procesar todos los correos si hay miles
      const ultimos = sequence.slice(-limit);
      
      // Si no hay suficientes correos, usar todos los disponibles
      const uidsParaObtener = ultimos.length > 0 ? ultimos : sequence;
...
async function obtenerCorreoPorUID(uid, carpeta = "INBOX") {
  ...
  try {
    await client.connect();
    console.log(`✅ Conectado al servidor IMAP. Buscando correo UID ${uid} en carpeta: ${carpeta}`);
    ...
    try {
      // Verificar que el correo existe antes de intentar obtenerlo
      const sequence = await client.search({ uid: uid });
      
      if (sequence.length === 0) {
        throw new Error(`Correo con UID ${uid} no encontrado en la carpeta ${nombreCarpetaReal}`);
      }
      ...
      for await (let msg of client.fetch([uid], {
        ...
        try {
          ...
          const fromText = parsed.from?.text || msg.envelope.from?.map(f => {
            if (f.name) {
              return `${f.name} <${f.address}>`;
            }
            return f.address;
          ...
          const toText = parsed.to?.text || msg.envelope.to?.map(t => t.address).join(", ") || "";
          ...
          console.log(`✅ Correo UID ${uid} obtenido exitosamente`);
          break; // Solo procesamos el primer mensaje (debería ser solo uno)
        } catch (parseError) {
          console.error(`⚠️ Error parseando correo UID ${msg.uid}:`, parseError.message);
          // Si falla el parseo, intentar con datos del envelope
          correoEncontrado = {
            uid: msg.uid,
            subject: msg.envelope.subject || "(Sin asunto)",
          ...
          console.log(`⚠️ Correo obtenido con datos del envelope (sin parsear contenido)`);
          break;
        }
      }
      
      if (!correoEncontrado) {
        throw new Error(`No se pudo obtener el correo con UID ${uid} de la carpeta ${nombreCarpetaReal}`);
      }
    ...
    console.error("❌ Error obteniendo correo por UID:", error.message);
    // Mejorar el mensaje de error para errores de conexión
    if (error.message.includes("Connection") || error.message.includes("NoConnection") || error.code === "NoConnection") {
