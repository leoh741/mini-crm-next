...
async function obtenerCorreoPorUID(uid, carpeta = "INBOX") {
  ...
  try {
    ...
    // Verificar que la carpeta existe y encontrar el nombre correcto
    let nombreCarpetaReal = carpeta;
    let carpetaExiste = false;
    ...
    try {
      ...
      // Intentar variaciones comunes del nombre de carpeta
      const variaciones = [
        carpeta,
        carpeta.toUpperCase(),
        carpeta.toLowerCase(),
    ...
    try {
      // Aquí sí obtenemos el source completo para parsear el contenido
      ...
      for await (let msg of messages) {
        try {
          ...
          throw new Error(`Error al parsear el correo: ${parseError.message}`);
        }
      }
