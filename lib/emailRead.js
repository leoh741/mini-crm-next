...
import { enqueueFetchBody, isUidInProgress } from "./emailBodyFetcher.js";
...
async function obtenerCorreoPorUID(uid, carpeta = "INBOX", incluirContenido = false) {
  ...
  if (cachedPersistente) {
    ...
    let bodyStatusActual = cachedPersistente.bodyStatus || (tieneContenido ? 'ready' : 'loading');
    
    // PROTECCIÓN: Si bodyStatus="loading" y lastBodyAttemptAt es muy viejo (>2 min), degradar a "error"
    if (bodyStatusActual === 'loading' && cachedPersistente.lastBodyAttemptAt) {
      const tiempoDesdeUltimoIntento = Date.now() - new Date(cachedPersistente.lastBodyAttemptAt).getTime();
      const MAX_LOADING_AGE_MS = 2 * 60 * 1000; // 2 minutos
      
      if (tiempoDesdeUltimoIntento > MAX_LOADING_AGE_MS) {
        console.log(`[email-open] ⚠️ Correo UID ${uid} tiene bodyStatus="loading" muy viejo (${Math.round(tiempoDesdeUltimoIntento / 1000)}s), degradando a "error"`);
        bodyStatusActual = 'error';
        
        // Actualizar en cache persistente
        try {
          await guardarCorreoEnCache(uidNumero, carpeta, cachedPersistente, false, 'error', 'Loading timeout (más de 2 minutos)');
        } catch (updateError) {
          console.warn(`[email-open] ⚠️ Error actualizando bodyStatus a error: ${updateError.message}`);
        }
      }
    }
    ...
    if (incluirContenido && !tieneContenido) {
      ...
      if (bodyStatusActual === 'error') {
        ...
        const correoConError = {
          ...
          lastBodyAttemptAt: cachedPersistente.lastBodyAttemptAt || null,
          text: "",
          html: "",
          attachments: []
        };
      ...
      // SALVAGUARDA: Si bodyStatus="loading" y lastBodyAttemptAt es viejo (>2 min) y NO hay inProgress, reencolar
      if (bodyStatusActual === 'loading') {
        const lastAttemptAt = cachedPersistente.lastBodyAttemptAt 
          ? new Date(cachedPersistente.lastBodyAttemptAt).getTime() 
          : null;
        const ahora = Date.now();
        const MAX_LOADING_AGE_MS = 2 * 60 * 1000; // 2 minutos
        const enProceso = isUidInProgress(uidNumero, carpeta);
        
        // Si no hay lastBodyAttemptAt o es muy viejo, y NO hay inProgress vigente
        if ((!lastAttemptAt || (ahora - lastAttemptAt > MAX_LOADING_AGE_MS)) && !enProceso) {
          console.log(`[email-open] 🔄 SALVAGUARDA: bodyStatus="loading" stale (lastAttemptAt=${lastAttemptAt ? Math.round((ahora - lastAttemptAt) / 1000) + 's' : 'null'}), reencolando con forzarReintento=true`);
          // Reencolar con forzarReintento=true
          enqueueFetchBody(uidNumero, carpeta, true);
        }
      } else {
        // Si no hay error previo y no está en loading, disparar descarga en background
      ...
      // Retornar correo con bodyStatus: "loading" (incluyendo lastBodyAttemptAt y lastBodyError si existen)
      const correoConLoading = {
        ...cachedPersistente,
        bodyStatus: "loading",
        lastBodyAttemptAt: cachedPersistente.lastBodyAttemptAt || null,
        lastBodyError: cachedPersistente.lastBodyError || null,
        text: "",
        html: "",
        attachments: []
      };
