...
export async function getEstadosPagoMes(clientesIds, mes, año, useCache = false) {
  try {
    const mesKey = getMesKey(mes, año);
...
export async function actualizarCliente(id, datosActualizados, limpiarCache = false) {
  if (limpiarCache) {
    limpiarCacheClientes(); // Limpiar cache solo si se solicita explícitamente
  }
  
  try {
    // Primero obtener el cliente para saber su _id de MongoDB (sin caché para obtener datos frescos)
    const cliente = await getClienteById(id, false);
