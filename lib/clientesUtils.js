...
// Obtener todos los pagos de un mes (incluye clientes borrados) para congelar ingresos
export async function getPagosMesCompleto(mes, año) {
  try {
    const mesKey = getMesKey(mes, año);
    const response = await fetch(`/api/pagos?mes=${mesKey}&incluirTodos=true`, {
      cache: 'no-store',
      signal: AbortSignal.timeout(30000)
    });

    if (!response.ok) {
      console.warn('Error al obtener pagos del mes:', response.status);
      return [];
    }

    const data = await response.json();
    if (data.success && Array.isArray(data.data)) {
      return data.data.filter(p => p.crmClientId !== '__month_init__');
    }
    return [];
  } catch (error) {
    console.error('Error al obtener pagos completos del mes:', error);
    return [];
  }
}

// Suma de ingresos automáticos congelados de un mes (snapshots montoPagado)
export async function getIngresosAutomaticosMes(mes, año) {
  const pagos = await getPagosMesCompleto(mes, año);
  return pagos.reduce((sum, pago) => sum + (Number(pago.montoPagado) || 0), 0);
}
...
export async function getMesesConRegistros() {
  try {
    ...
    if (!response.ok) {
      console.warn('Error al obtener meses con registros:', response.status);
      return [];
    }
    ...
    if (error.name === 'AbortError') {
      console.warn('Timeout al obtener meses con registros');
    } else {
...
export async function getClientes(forceRefresh = false) {
  try {
    ...
    if (data.success) {
      ...
      // PROTECCIÓN: Si el servidor está vacío pero tenemos datos en localStorage, usar localStorage
      if (clientes.length === 0 && localClientes && localClientes.length > 0) {
        console.warn('⚠️ Base de datos vacía pero hay datos en localStorage. Usando localStorage como respaldo.');
        return applyPendingUpdates(localClientes);
      }
    ...
    // Si la respuesta no es exitosa pero tenemos localStorage, usarlo como respaldo
    if (localClientes && localClientes.length > 0) {
      console.warn('⚠️ Error en respuesta del servidor pero hay datos en localStorage. Usando localStorage como respaldo.');
      return applyPendingUpdates(localClientes);
    }
...
export async function agregarCliente(cliente) {
  const clienteData = {
    id: cliente.id || `client-${Date.now()}-${Math.random().toString(36).slice(2, 11)}`,
