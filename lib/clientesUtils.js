...
export async function guardarEstadoPagoServicio(clienteId, mes, año, indiceServicio, pagado) {
  ...
  const estadoActual = await getEstadoPagoMes(clienteId, mes, año, true) || {};
  const serviciosPagados = estadoActual.serviciosPagados || {};
  ...
  const cliente = await getClienteById(clienteId, true);
  const todosLosServiciosPagados = cliente && cliente.servicios 
    ? cliente.servicios.every((_, idx) => serviciosPagados[idx] === true)
    : false;
  
  // Guardar con todos los servicios actualizados
  return await guardarEstadoPagoMes(clienteId, mes, año, todosLosServiciosPagados, serviciosPagados);
}
...
async function syncClienteUpdateToServer(mongoId, datosActualizados, clienteId, clienteAnterior) {
  try {
    ...
    let response = await fetch(`/api/clientes/${mongoId}`, {
      ...
      signal: AbortSignal.timeout(5000) // Timeout adecuado para servidor local
    });
